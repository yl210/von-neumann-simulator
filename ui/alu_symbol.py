...
class ALUSymbol(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # ALU polygon
        points = QPolygon([
            QPoint(int(w * 0.25), int(h * 0.15)), 
            QPoint(int(w * 0.4), int(h * 0.15)),
            QPoint(int(w * 0.5), int(h * 0.6)), 
            QPoint(int(w * 0.6), int(h * 0.15)),
            QPoint(int(w * 0.75), int(h * 0.15)),  
            QPoint(int(w * 0.60), int(h * 0.85)),
            QPoint(int(w * 0.40), int(h * 0.85)),  
        ])
    ...
    # updates alu color if selected
    def update_alu_color(self, color):
        self.fill_color = tuple(bytes.fromhex(color))
        self.update()
    
    # updates alu inputs depending on demux outputs
